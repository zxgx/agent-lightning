...
class OfflineRLOOTrainer(RLOOTrainer):
    """
    ...
    def __init__(
        ...
        logger.info(f"RLOO Config: {pformat(args.to_dict(), sort_dicts=False)}")
        ...
        self.batch_shuffle_rng = torch.Generator()
        self.batch_shuffle_rng.manual_seed(args.seed + self.accelerator.torch_device_mesh["dp"].get_local_rank() * 100)
        ...
        if self.use_vllm:
            ...
            logger.info(f">>> Using offline data generated from vLLM!!!", main_process_only=False)
    ...
    def get_train_dataloader(self):
        ...
        dataloader_params = {
            ...
            * self.args.steps_per_generation,  # < this is the change # _train_batch_size = per_device_train_batch_size * num_processes
    ...
    def _get_train_sampler(self, dataset: Optional[Dataset] = None) -> Sampler:
        ...
        return RepeatSampler(
            data_source=dataset,
            mini_repeat_count=self.num_generations,  # generations per prompt
            batch_size=self.args.generation_batch_size
            // self.num_generations,  # generation_batch_size = per_device_train_batch_size * num_processes * steps_per_generation
    ...
    def _get_per_token_logps_and_entropies(
        ...
        # required by context parallel
        if self.accelerator.parallelism_config.cp_size > 1:
            assert "logits_to_keep" in self.model_kwarg_keys, f"unhandled logics for padding without logits_to_keep"
            pad_seq_len = self.accelerator.parallelism_config.cp_size * 2
            input_ids = pad(input_ids, self.pad_token_id, pad_to_multiple_of=pad_seq_len, padding_side="left")
            attention_mask = pad(attention_mask, 0, pad_to_multiple_of=pad_seq_len, padding_side="left")
        ...
        for start in range(0, input_ids.size(0), batch_size):
            ...
            # Enable context parallelism if applicable
            with self.accelerator.maybe_context_parallel(
                buffers=[model_inputs["input_ids"], model_inputs["attention_mask"]],
                buffer_seq_dims=[1, 1],
                # no_restore_buffers={model_inputs["input_ids"]},
            ):
    ...
    def _prepare_inputs(
        ...
        if mode == "train":
            ...
            if self._step % generate_every == 0 or self._buffered_inputs is None:
                ...
                generation_batch = self.stateful_shuffle_sequence_dict(generation_batch)
    ...
    def stateful_shuffle_sequence_dict(self, seq_dict: dict[str, Optional[Sequence]]) -> dict[str, Optional[Sequence]]:
        # Determine batch size from the first non-None sequence
        batch_size = len(next(v for v in seq_dict.values() if v is not None))
        permutation = torch.randperm(batch_size, generator=self.batch_shuffle_rng)
    ...
    def _generate_and_score_completions(
        ...
        # construct prompt ids and mask
        prompt_ids = [torch.tensor(x["prompt_ids"], device=device) for x in inputs]
        prompt_ids = pad(prompt_ids, padding_value=self.pad_token_id, padding_side="left")
        prompt_mask = [torch.ones(len(x["prompt_ids"]), device=device) for x in inputs]
        prompt_mask = pad(prompt_mask, padding_value=0, padding_side="left")

        prompt_ids, prompt_mask = Trainer._prepare_inputs(self, (prompt_ids, prompt_mask))

        prompts_text = [x["prompt"] for x in inputs]

        # use offline completion ids and mask generated from vllm
        completion_ids = [torch.tensor(x["gold_completion_ids"], device=device) for x in inputs]
        completion_ids = pad(completion_ids, padding_value=self.pad_token_id)

        completion_ids = Trainer._prepare_inputs(self, completion_ids)

        # concatenate prompt and completion ids for logit computation
        ...
        logprobs = []
        for x in inputs:
            min_prob = min(min_prob, min(x["logprobs"]))
            logprobs.append(torch.tensor(x["logprobs"], dtype=torch.float32, device=device))
        logprobs = pad(logprobs, padding_value=min_prob)

        old_per_token_logps = logprobs
        old_logps = (old_per_token_logps * completion_mask).sum(1)

        if self.beta != 0.0:
            ref_per_token_logps = logprobs.clone()
        else:
        ...
        mean_grouped_rewards = grouped_rewards.mean(dim=1)  # [num_prompts, ]
        # std_rewards = grouped_rewards.std(dim=1)                  # [num_prompts, ]
        std_rewards = torch.zeros_like(mean_grouped_rewards)  # TODO: temporary fix for num_generations=1
        is_std_zero = torch.isclose(std_rewards, torch.zeros_like(std_rewards))

        # RLOO advantages computation
        # TODO: temporary fix for num_generations=1
    ...
    def __generate_and_score_completions(
        self, inputs: list[dict[str, Union[torch.Tensor, Any]]]
    ) -> dict[str, Union[torch.Tensor, Any]]:
        device = self.accelerator.device
        mode = "train" if self.model.training else "eval"
    ...
    def _save_checkpoint(self, model, trial):
        ...
        Trainer._save_checkpoint(self, model, trial)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        print(f"Saving model of type {type(self.model)}")
